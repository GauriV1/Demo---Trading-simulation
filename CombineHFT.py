...
def run_trading_simulation(
    ...
    displaying plots in Streamlit via st.pyplot().
    """
    ...
    # Often returns the newest date first, so sort ascending
    ...
    # ------------------------------------------------------------
    # 2. Ensure a 'Date' column exists
    # ------------------------------------------------------------
    # Store the old index (DatetimeIndex) into a new 'Date' column
    ...
    plt.tight_layout()
    st.pyplot(fig)  # <--- streamlit plotting
...
if __name__ == "__main__":
    ...
    # Provide your Alpha Vantage API key or have the user input it
    alpha_key_input = st.text_input("Alpha Vantage API Key", "YOUR_ALPHA_VANTAGE_API_KEY", type="password")
    
    # Could also allow user to select risk tolerance
