...
# Simplified function to generate buy/sell signals based solely on EMA crossovers
def generate_signals_simple(data):
    """
    Generates buy and sell signals based solely on EMA crossovers for testing.
    ...
    # Buy Conditions
    buy_condition = (data['EMA_Crossover_Signal'] == 2)  # EMA5 crossed above EMA15
    data.loc[buy_condition, 'Signal'] = 1

    # Sell Conditions
    sell_condition = (data['EMA_Crossover_Signal'] == -2)  # EMA5 crossed below EMA15
    data.loc[sell_condition, 'Signal'] = -1

    # Debugging: Show the last few signals
    st.write("### Recent Signals and Indicators")
    st.write(data[['EMA5', 'EMA15', 'Signal']].tail())
...
def simulate_trades(data, price_column, initial_capital=100000, transaction_cost=10,
                   stop_loss_multiplier=1.0, risk_to_reward=1.5, risk_percentage=0.5):
    """
...
# Function to plot MACD and OBV indicators
def plot_additional_indicators(data):
    """
    Plots MACD and OBV indicators for deeper analysis.
...
# Function to plot MACD and OBV using Plotly
...
def main():
    ...
    st.title("📈 High-Frequency Trading Simulation with EMA Crossovers")
    st.markdown("""
    ...
    **Note:** Due to data limitations, minute-level data is available only for the past 7 days.
    ...
    stop_loss_multiplier = st.sidebar.slider("Stop-Loss ATR Multiplier", min_value=1.0, max_value=5.0, value=1.0, step=0.1)
    risk_to_reward = st.sidebar.slider("Risk to Reward Ratio", min_value=1.0, max_value=5.0, value=1.5, step=0.1)

    # Risk percentage per trade
    risk_percentage = st.sidebar.slider("Risk Percentage per Trade (%)", min_value=0.1, max_value=5.0, value=0.5, step=0.1)
    ...
    with st.spinner('Fetching and processing data...'):
        ...
        data = generate_signals_simple(data)
    ...
    with tab2:
        ...
        plot_method = st.radio("Choose Plotting Method for Additional Indicators", ["Matplotlib", "Plotly"])
    ...
    with tab3:
        ...
        # Choose plotting method
        plot_method_signals = st.radio("Choose Plotting Method for Buy/Sell Signals", ["Matplotlib", "Plotly"])
    ...
    with tab4:
        ...
        # Choose plotting method
        plot_method_portfolio = st.radio("Choose Plotting Method for Portfolio Value", ["Matplotlib", "Plotly"])
    ...
    if st.checkbox("Show Data with Signals"):
        ...
        st.write(data[[price_column, 'EMA5', 'EMA15', 'RSI', 'Signal', 'Portfolio_Value']])
