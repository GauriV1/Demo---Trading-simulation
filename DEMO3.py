...
def calculate_indicators(data, price_column):
    ...
    data['EMA9'] = trend.ema_indicator(close=data[price_column], window=9)
    data['EMA21'] = trend.ema_indicator(close=data[price_column], window=21)
...
def generate_signals(data):
    """
    Generates buy and sell signals based on EMA crossovers and MACD.
    ...
    data['EMA_Crossover'] = np.where(data['EMA9'] > data['EMA21'], 1, -1)
    ...
    # Buy Conditions
    buy_condition = (
        (data['EMA_Crossover_Signal'] == 2) &  # EMA9 crossed above EMA21
    ...
    # Sell Conditions
    sell_condition = (
        (data['EMA_Crossover_Signal'] == -2) &  # EMA9 crossed below EMA21
        (data['RSI'] > 65) &  # Higher RSI threshold for more sells
        (data['MACD_Crossover_Signal'] < 0)  # MACD crossed below signal line
    )
    data.loc[sell_condition, 'Signal'] = -1

    # Debugging: Show some signals
    st.write(data[['EMA9', 'EMA21', 'RSI', 'MACD', 'MACD_Signal', 'Signal']].tail())

    return data

# Function to simulate trades with risk management and transaction costs
def simulate_trades(data, price_column, initial_capital=100000, transaction_cost=10, stop_loss_multiplier=1.5, risk_to_reward=2.0, risk_percentage=1.0):
    """
...
def plot_results(data, trades_df, price_column):
    ...
    if not trades_df.empty:
        ...
        st.warning("No trades were executed to plot buy/sell signals.")
...
def plot_indicators(data, price_column):
    ...
    # Plot EMA9 and EMA21
    ax1.plot(data.index, data['EMA9'], label='EMA9', color='blue', linestyle='--')
    ax1.plot(data.index, data['EMA21'], label='EMA21', color='orange', linestyle='--')
    ax1.legend(loc='upper left')
    ...
    ax2.axhline(35, color='red', linestyle='--', linewidth=0.5)
    ax2.axhline(65, color='red', linestyle='--', linewidth=0.5)
    ax2.set_ylabel('RSI', color='green')
    ax2.tick_params(axis='y', labelcolor='green')
    ax2.legend(loc='upper right')

    plt.title('Price with EMA9, EMA21 and RSI')
    plt.tight_layout()
    st.pyplot(fig)
...
def main():
    ...
    st.title("📈 High-Frequency Trading Simulation with EMA, MACD, and ATR-based TP/SL")
    st.markdown("""
    ...
    **Note:** Due to data limitations, minute-level data is available only for the past 7 days.
    ...
    if st.checkbox("Show Data with Signals"):
        ...
        st.write(data[[price_column, 'EMA9', 'EMA21', 'MACD', 'MACD_Signal', 'ATR14', 'Signal', 'Portfolio_Value']])
